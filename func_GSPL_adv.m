function [W_hat, p_vec, z_vec] = func_GSPL_adv(X_v, label, n, s, c,...
...
%% function body
    ...
    [n, d_tol] = size(X_hat);
    X_hat_mean_c = zeros(max(label), d_tol);
    counter_c = zeros(1, max(label));
    for i = 1 : n
        X_hat_mean_c(label(i), :) = X_hat_mean_c(label(i), :) + X_hat(i, :);
        counter_c(label(i)) = counter_c(label(i)) + 1;
    end
    X_hat_mean_c = inv(diag(counter_c)) * X_hat_mean_c;
    X_wi = X_hat;
    for i = 1 : n
        X_wi(i, :) = X_wi(i, :) - X_hat_mean_c(label(i));
    end
